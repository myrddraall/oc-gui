-- alternative CSS parser declaration
--
...
function parser:parse_strings(text)
  ...
  local function replace_string(s)
    ...
    print("string", count, self.strings[count])
    return "%%" .. count
  end
...
function parser:parse_statements(text)
  ...
  while pos do
    ...
    if startpos then
      ...
      if at == "@" then 
        obj, pos = self:parse_at_rule(text, startpos)
        print(obj.statement)
      else
        obj, pos = self:parse_rule(text, startpos)
        print(obj.selector)
        for k,v in pairs(obj.declarations) do
